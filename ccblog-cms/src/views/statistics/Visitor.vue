    ...
    <el-table :data="visitorList">
      ...
      <el-table-column label="操作" width="200">
        <template v-slot="scope">
          ...
          <el-popconfirm title="确定删除吗？" icon="el-icon-delete" iconColor="red" @confirm="deleteVisitorById(scope.row)">
...
export default {
  name: "Visitor",
  components: {Breadcrumb,DateTimeRangePicker},
  data(){
    return {
      queryInfo:{
        date:[],
  ...
  methods:{
    ...
    deleteVisitorById(row){
      deleteVisitor(row.id,row.uuid).then(res => {
        if (res.code === 200){
          this.getData()
