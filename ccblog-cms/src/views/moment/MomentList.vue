      ...
      <el-table-column label="发布状态" width="80">
        <template v-slot="scope">
          <el-switch v-model="scope.row.published"
                     @change="momentStatusChange(scope.row)"
          ></el-switch>
        </template>
      </el-table-column>
...
import {getMomentList,updateMomentPublished,deleteMoment} from "@/api/moment"
export default {
  name: "MomentList",
  ...
  methods:{
    getData(){
      getMomentList(this.queryInfo).then(res => {
    ...
    momentStatusChange(row){
      console.log(row)
      updateMomentPublished(row.id,row.published).then(res => {
        this.msgSuccess(res.msg)
      })
    },
    goMomentEditPage(id){
      this.$router.push(`/moment/edit/${id}`)
    },
    handleDelete(id){
