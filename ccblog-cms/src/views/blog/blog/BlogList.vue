  ...
  <div class="app-container">
    ...
    <el-table :data="blogList">
      ...
      <el-table-column label="操作" width="200">
        <template v-slot="scope">
          ...
          <el-button size="mini" type="danger" icon="el-icon-delete" slot="reference" @click="handleDelete(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="博客可见性" width="30%" :visible.sync="dialogVisible">
      ...
      <el-form label-width="50px" @submit.native.prevent>
        <el-form-item>
          <el-radio-group v-model="visForm.published">
            <el-radio :label="true">公开</el-radio>
            <el-radio :label="false">私密</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="visForm.published">
          <el-row>
            <el-col :span="6">
...
import {getBlogInfoByQuery,updateBlogTop,updateBlogRecommend,deleteBlogById,updateBlogVisibility} from "@/api/blog";
...
export default {
  ...
  methods: {
    getData() {
      getBlogInfoByQuery(this.queryInfo).then(res => {
    ...
    blogTopChanged(row){
      updateBlogTop(row.id,row.top).then(res => {
        this.msgSuccess(res.msg)
      })
    },
    blogRecommendChanged(row){
      updateBlogRecommend(row.id,row.recommend).then(res => {
        this.msgSuccess(res.msg)
      })
    },
    handleDelete(id){
