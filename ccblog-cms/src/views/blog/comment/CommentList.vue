    ...
    <el-row>
      <el-col :span="6">
        <el-select v-model="pageId" placeholder="请选择页面" :filterable="true" :clearable="true" @change="search" size="small" style="min-width: 400px">
          <el-option :value="item.id" v-for="item in blogList" :label="item.title" :key="item.id"></el-option>
        </el-select>
      </el-col>
    </el-row>
    <el-row>
        <el-table :data="commentList" row-key="id" :tree-props="{children: 'replyComments'}">
          ...
          <el-table-column label="所在页面" show-overflow-tooltip>
            <template v-slot="scope">
              <a type="success" :href="`https://kudaompq.top/blog/${scope.row.blog.id}`" target="_blank" v-if="scope.row.page===0">{{ scope.row.blog.title }}</a>
              <a type="success" href="https://kudaompq.top/about" target="_blank" v-else-if="scope.row.page===1">关于我</a>
              <a type="success" href="https://kudaompq.top/friends" target="_blank" v-else-if="scope.row.page===2">友人帐</a>
            </template>
          </el-table-column>
...
<script>
import {getCommentListByPage,getBlogList,updatePublished,updateNotice,updateComment,deleteComment} from "@/api/comment";
...
export default {
  ...
  methods:{
    getCommentList(){
      getCommentListByPage(this.queryInfo).then(res =>{
        if (res.code === 200){
