  ...
  <div class="app-container">
    <el-form inline>
      ...
      <el-form-item style="margin-left: 20px">
        <el-switch v-model="friendInfo.commentEnabled" active-text="页面评论" @change="commentEnabledChanged"></el-switch>
      </el-form-item>
    </el-form>
    <el-table :data="friendList">
      ...
      <el-table-column label="操作" width="200">
        <template v-slot="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDialog(scope.row)" style="margin-right: 10px">编辑</el-button>
    ...
    <el-form label-position="top">
      <el-form-item label="友链页面信息">
        <mavon-editor v-model="friendInfo.content"/>
      </el-form-item>
...
import {getFriendList,
  getFriendInfo,
  updateFriendInfoCommentEnabled,
  updateFriendPublished,
  deleteFriend,
  updateFriendInfoContent,
  saveFriend,
  updateFriend} from "@/api/friend";
...
export default {
  ...
  methods:{
    getList(){
      getFriendList(this.queryInfo).then(res =>{
        if (res.code === 200){
    ...
    commentEnabledChanged(commentEnabled){
      updateFriendInfoCommentEnabled(commentEnabled).then(res => {
        if (res.code === 200){
    ...
    friendPublishedChanged(row){
      updateFriendPublished(row.id,row.published).then(res =>{
        if(res.code === 200){
          this.msgSuccess(res.msg)
        }
      })
    ...
    updateContent(){
      updateFriendInfoContent(this.friendInfo.content).then(res =>{
        if (res.code === 200){
          this.msgSuccess(res.msg)
          this.getInfo()
        }
